...
def plot_ellipse(semimaj=1, semimin=1, phi=0, x_cent=0, y_cent=0, theta_num=1e3, ax=None, plot_kwargs=None, fill=False, fill_kwargs=None, data_out=False, cov=None, mass_level=0.68):
    '''
    ...
    if cov is not None:
        ...
        if mass_level is None:
            ...
            multiplier = np.sqrt(distances[np.where(
                np.abs(chi2_cdf - mass_level) == np.abs(chi2_cdf - mass_level).min())[0][0]])
        semimaj *= multiplier
        semimin *= multiplier
...
plot_ellipse(semimaj=2, semimin=1, phi=np.pi / 4, x_cent=2, y_cent=2,
             ax=ax, plot_kwargs=plot_kwargs, fill=False, fill_kwargs=fill_kwargs)

plot_kwargs = {'color': 'b', 'linestyle': '-', 'linewidth': 3, 'alpha': 0.8}
fill_kwargs = {'color': 'b', 'alpha': 0.7}
plot_ellipse(semimaj=1, semimin=1, phi=0, x_cent=0, y_cent=0,
