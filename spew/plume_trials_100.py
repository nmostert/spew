...
units = "km"
trial = 8
vals = np.linspace(10000, 40000, 101)
...
def gamma_der(x, a, b, c):
    return c * np.exp(bx) x ^ (-1 + a)(a + b x)
...
fit_df = pd.DataFrame(columns=['H', 'Phi', 'a', 'b', 'c', 'Success'])
...
for k, phi in enumerate(phis):
    ...
    for i, (h, erp) in enumerate(eruptions.items()):
        ...
        fit_df = fit_df.append(
            {'H': h, 'Phi': phi, 'a': popt.x[0], 'b': popt.x[1], 'c': popt.x[2], 'Success': popt.success}, ignore_index=True)
        # plt.show()
    # plt.tight_layout()
    # plt.show()
...
for i, piv, p in zip([0, 1, 2], [a_piv, b_piv, c_piv], ['a', 'b', 'c']):
    c = axs[i].imshow(piv.values, cmap='hot')
